...
export default function Legend({ open, visibleCategories, onToggle, onCategoryToggle, embedded }: Props) {
  useEffect(() => {
  ...
  if (embedded) {
    return (
      <nav className="w-full h-full flex flex-col overflow-y-auto" aria-label="Category filter">
        ...
        <ul className="px-3 pb-3 space-y-0.5 flex-1">
          {CATEGORIES.map((cat) => {
            ...
            return (
              <li key={cat.id}>
                <button
                  ...
                  style={{ backgroundColor: active ? `${cat.color}18` : 'transparent' }}
                >
                  <span
